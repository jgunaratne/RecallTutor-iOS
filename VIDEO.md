# Lecture Video Generation — Implementation Guide

## Goal

Add a **Video** button to the lecture card navigation bar in `MessageView.swift`. When tapped, generate a **~1-minute educational video** with:

1. **8 chained Veo segments** (8 seconds each = 64 seconds total) — each segment uses the last frame of the previous as its seed image
2. **AI-generated narration script** — Gemini creates a ~1-minute voiceover script from the lecture cards
3. **Text-to-speech voiceover** — Gemini `gemini-3.1-flash-tts-preview` API renders the narration to audio
4. **Final mux** — `AVMutableComposition` combines the stitched video + voiceover audio
5. **Full-screen player** — native `AVKit.VideoPlayer` presents the result

Store the final video locally on the phone (temp directory).

## Architecture

```
Video Button tap
  → 1. Gemini generates narration script + 8 scene prompts (JSON)
  → 2. For each of 8 segments:
       → Call Veo API with scene prompt + seed image (card illustration or last frame)
       → Poll LRO every 15s until done
       → Save segment .mp4, extract last frame for next segment
  → 3. AVMutableComposition concatenates 8 segments into one video
  → 4. Gemini TTS API renders narration → .wav audio file
  → 5. AVMutableComposition muxes video + audio → final .mp4
  → 6. Present full-screen AVPlayer
```

**Estimated generation time: 10–30 minutes** (Veo takes 1–4 min per segment × 8 segments).

---

## File 1: CREATE `RecallTutor/Services/VideoService.swift`

Ported from `../veoclip/backend/src/services/veo.service.ts` (Gemini API path). Adds segment chaining from VeoClip's `runPipeline` and on-device TTS + muxing.
...
## File 2: CREATE `RecallTutor/Views/VideoPlayerView.swift`
...
## File 3: MODIFY `RecallTutor/Views/MessageView.swift`

### 3a. Add state properties to `LectureView` (after `imageGenerator` on line 14)
...
### 3b. Add Video button in the navigation bar

In the glass HStack (line 115), **after** the VoiceControlBar block (line 134–136) and **before** the second `Spacer()` at line 138, insert:
...
### 3c. Add modifiers after the last `.onChange` block (~line 197)
...
### 3d. Add helper methods (before `loadingCard`, ~line 200)
...
Add these new files to the `RecallTutor` target:
- `RecallTutor/Services/VideoService.swift`
- `RecallTutor/Views/VideoPlayerView.swift`

---

## Key References

| Concept | VeoClip Source | iOS Equivalent |
|---------|---------------|----------------|
| Veo API call | `veo.service.ts:generateVideoGemini` (L218–348) | `VideoService.generateSingleSegment` |
| LRO polling | `veo.service.ts` (L275–344) — GET on operation URL | Same pattern with URLSession |
| Segment chaining | `api.ts:runPipeline` — extract last frame, use as next seed | `extractLastFrame` via `AVAssetImageGenerator` |
| Concat segments | `mux.service.ts:concatenateVideos` — ffmpeg concat demuxer | `AVMutableComposition` |
| TTS | `tts.service.ts` — `@google-cloud/text-to-speech` SDK | `gemini-3.1-flash-tts-preview` API via `generateContent` |
| Mux video+audio | `mux.service.ts:muxVideoAndAudio` — ffmpeg `-shortest` | `AVMutableComposition` + `AVAssetExportSession` |

## Testing Checklist

1. **No API key** → Video button hidden
2. **Tap Video** → "Writing narration script…" overlay appears
3. **Script generated** → "Generating scene 1 of 8" with progress bar
4. **Each segment** → Counter updates ("scene 2 of 8", etc.)
5. **Cancel mid-generation** → Overlay dismisses, tasks cancelled, temp files cleaned
6. **All segments done** → "Stitching scenes together…" → "Recording narration…" → "Finalizing video…"
7. **Success** → Full-screen video player opens, video plays with voiceover
8. **Veo error** → Error message in overlay with Close button
9. **Content filtered** → Specific "Content filtered" error message
